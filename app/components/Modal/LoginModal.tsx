...
const LoginModal: React.FC<{
  ...
  const { appState, setAppState ,setIsLoggedIn} = useContext(AppContext);
  //@ts-ignore
...
console.log(res)
  ...
  // const handleLoginAtl = () => {
  //   window.location.href = `https://auth.atlassian.com/authorize?client_id=${process.env.ATLAS_CLIENT_ID}&redirect_uri=${encodeURIComponent(process.env.ATLAS_REDIRECT_URI)}&scope=read%3Ajira-user&response_type=code&prompt=consent`;
  // };
  return (
    <>
...
setInputVal({
 ...
 <label htmlFor="" className="text-red-300">{errMessage}</label>  </div>
  ...
  onClick={()=>{
    setIsLoading(true)
    handleLogin({onSuccess:onSuccess,onError:onError,user:inputVal})
  }}
